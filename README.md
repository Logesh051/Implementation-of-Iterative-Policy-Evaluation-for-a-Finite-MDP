...
This project implements **Iterative Policy Evaluation** for a **Finite Markov Decision Process (MDP)** using the **FrozenLake-v1** environment from Gymnasium. The objective is to estimate the state-value function for a fixed random policy using the Bellman Expectation Equation.
...
7. Display the final value function as a **4 × 4** grid.

---

## Program

The complete implementation is available in **PolicyEvaluationExp.ipynb**.
...
The iterative policy evaluation algorithm successfully computes the state-value function for all states under a fixed random policy. The values converge after repeated Bellman updates, demonstrating how policy evaluation estimates the expected return for each state.
